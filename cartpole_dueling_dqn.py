...
class DuelingDQNetwork(nn.Module):
    def __init__(self, lr, input_dims, fc1_dims, fc2_dims, n_actions):
        ...
        self.V = nn.Linear(fc2_dims, 1) # VALUE STREAM: outputs a single number 
        self.A = nn.Linear(fc2_dims, n_actions) # ADVANTAGE STREAM: outputs one value per action 
...
class DuelingDQNAgent:
    def __init__(self, gamma=0.99, epsilon=1.0, lr=0.001, input_dims=4,
        ...
        self.Q_eval = DuelingDQNetwork(lr, input_dims, fc1_dims=128, fc2_dims=128, 
                                      n_actions=n_actions)
        self.Q_target = DuelingDQNetwork(lr, input_dims, fc1_dims=128, fc2_dims=128, 
                                        n_actions=n_actions)
    ...
    def choose_action(self, observation):
        if np.random.random() > self.epsilon:
            ...
            actions = self.Q_eval.forward(state)  # Q = V + (A - mean(A))
...
def train_dueling_dqn(episodes=500):
    env = gym.make('CartPole-v1')
    agent = DuelingDQNAgent(gamma=0.99, epsilon=1.0, lr=0.001, input_dims=4,
...
if __name__ == '__main__':
    scores, eps_history = train_dueling_dqn(episodes=500)
    
    plt.figure(figsize=(12, 5))
    plt.subplot(1, 2, 1)
    plt.plot(scores)
    plt.title('Dueling DQN: Scores over Episodes')
    plt.xlabel('Episode')
    plt.ylabel('Score')
